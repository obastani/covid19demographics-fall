...
def new_form(soup, soup2, matched=False):
    x = soup.find("div", class_="ms-rtestate-field")
    m = None
    while m is None:
        oldx = x
        x = x.find_next("div", class_="NewsItemContent")
        txt = x.text.replace("\xa0", " ") if x is not None else None
        if x is None: 
            x = oldx.find_next("div", class_="ms-rtestate-field")
            txt = x.text.replace("\xa0", " ").replace("\r\n", " ")
        elif not txt:
            x = x.find_next("p")
            txt = x.text.replace("\xa0", " ")
        m = re.search("now has ([0-9,\,]+) confirmed cases and ([0-9,\,]+)", txt)
    m = m.groups()
    deaths = stoi(m[1])
    cases = stoi(m[0])
    x = x.find_next("a")
    m = re.search("As of ([A-Z][a-z]+ [0-9]+), approximately ([0-9,\,]+)[+]*[*]* tests .* At least ([0-9,\,]+) results have been received and another ([0-9,\,]+) are pending.", txt)
    if m is None: 
        #x = x.find_next("div", class_="NewsItemContent")
        txt = soup.text.replace("\x0a", " ").replace("\r", "") # screw it, look through everything
        m = re.search("As of ([A-Z][a-z]+ [0-9]+), approximately ([0-9,\,]+)[+]*[*]* tests .* At least ([0-9,\,]+) results have been received and another ([0-9,\,]+) are pending.", txt)
    m = m.groups()
    date = m[0]
    tests = stoi(m[1])
    confirmed = stoi(m[2])
    pending = stoi(m[3])
    x = soup2.find(text="COVID-19 by the Numbers").parent.parent.find_next("p")  
    m = re.search("As of ([A-Z][a-z]+ [0-9]+)", x.text.replace("\xa0", " "))
    if m is None:
        # Just look at everything...
        x = soup2
        m = re.search("As of ([A-Z][a-z]+ [0-9]+)", x.text.replace("\xa0", " "))
    if m.groups()[0] == date or not matched:
        x = x.find_next("ul")
        subtxt = x.text.replace("\xa0", " ") if x is not None else soup2.text.replace("\xa0", " ")
        a17 = stoi(re.search("Age 0-17:[ ]+([0-9,\,]+)", subtxt).groups()[0])
        au = stoi(re.search("Unknown[/Missing]*: ([0-9,\,]+)", subtxt).groups()[0])
        m = re.search("Age 18-49: ([0-9,\,]+)", subtxt)
        a1849 = stoi(m.groups()[0]) if m is not None else np.nan
        m = re.search("Age 50-64: ([0-9,\,]+)", subtxt)
        a5064 = stoi(m.groups()[0]) if m is not None else np.nan
        a65 = stoi(re.search("Age 65 and older..([0-9,\,]+)", subtxt).groups()[0])
        m = re.search("Age 18-64: ([0-9,\,]+)", subtxt)
        a1864 = stoi(m.groups()[0]) if m is not None else a1849+a5064
        subtxt = x.text.replace("\xa0", " ") if x is not None else subtxt 
        m = re.search("Female: ([0-9,]+) cases", subtxt)
        while m is None: 
            x = x.find_next("ul")
            subtxt = x.text.replace("\xa0", " ")
            m = re.search("Female: ([0-9,]+) cases", subtxt)
        m = m.groups()
        f_cases = stoi(m[0])
        m = re.search("Male: ([0-9,]+)", subtxt).groups()
        ...
        m = re.search("Unknown: ([0-9,]+)", subtxt).groups()
        u_cases = stoi(m[0])
        return (cases, deaths, f_cases, m_cases, u_cases, a17, a1849, a5064, a1864,a65, au, tests, 
                confirmed, pending, date, matched)
    else: 
        return (cases, deaths, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, tests, 
                confirmed, pending, date, matched)
...
def run_CA(args):
    ...
    immunization_url = "https://www.cdph.ca.gov/Programs/CID/DCDC/Pages/Immunization/ncov2019.aspx" 
    matched=False
    for x in soup.find_all("th", {'class':["ms-rteTableFirstCol-default", "ms-rteTableHeaderFirstCol-default"]}):
        ...
        # get date
        f = subsoup.find_all("div", class_="NewsItemContent")
        if len(f) > 1:
            txt = f[1].text
        else: 
            txt = subsoup.find("div", class_="ms-rtestate-field").text
        now = datetime.now()
        if re.search(emergency_file, txt) is not None:
            ...
            if m is not None:
                ...
                if subsoup.find(text="State Officials Announce Latest COVID-19 Facts") is None:
                    continue
                html = urlopen(immunization_url).read()
                soup = BeautifulSoup(html, "html.parser")
                g = open('{}/immunization_{}.html'.format(raw_name, now), 'w')
                g.write(str(soup))
                g.close()
                (pos, death, fc, mc, uc, a17, a1849, a5064, a1864,a65, au, tests, 
                    testRes, testPend, day, m) = new_form(subsoup, soup, matched)
